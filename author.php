...
<head>
    ...
    <?php
    if (isset($_GET['name'] , $_GET['id']) && !empty($_GET['name']) && !empty($_GET['id'])) {
      require_once 'admin/connect.php'; 
      $authName = $_GET['name'];
      $id = $_GET['id'];
      $getauthdt = $db->prepare("SELECT * FROM authors WHERE ID = $id");
      if($getauthdt->execute()){
          foreach ($getauthdt as $auth) {
          $id = $auth['ID'];
          $view = $auth['view'];
          ...
          echo '
          <title>مكتبة خيال | الكاتب : '.$AuthName.'</title>
          <meta name="description" content="'.$AuthDescription.'"/>
          <meta name="author" content="إسم الكاتب"/>
          ';
        }
      }
    }
...
<nav class="navbar navbar-white bg-white px-0 mb-4">
    ...
    <div class="d-flex">
    <figure>
      <img src="'.$AuthCover.'" class="card-img" />
    </figure>
      <div class="card-body">
        <h3 class="card-title mb-4">'.$AuthName.'</h3>
        <p class="card-title mb-4">'.$AuthDescription.'</p>
    </div>
  </div>
    </div>
            <hr class="border border-1"/>
            <h3>مؤلفاته</h3>
    <iframe src="'.$view.'" frameborder="0" width="100%" height="auto"></iframe>
            </div>
            ';
